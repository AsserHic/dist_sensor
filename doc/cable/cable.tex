...
\begin{frame}[t]{What are the wires}
\begin{columns}[t]
 \begin{column}{0.48\textwidth}
   \begin{itemize}
...
\begin{columns}
  ...
  \begin{column}{0.44\textwidth}
   \tcbox[colframe=green!30!black, colback=green!30]{
    \includegraphics[width=0.75\linewidth]{connectors.jpg}
  }
  \end{column}
\end{columns}
\end{frame}
...
\only<1>{{\footnotesize Veritasium introduced an interesting setup to demonstrate how the signal propagates within an electrical circuit~\cite{veritasium2021}.
...
\only<2->{
 \begin{enumerate}[A.]
  ...
  \item<4-> \only<5>{\color{green}}$\frac{\SI{1}{\meter}}{c}$
